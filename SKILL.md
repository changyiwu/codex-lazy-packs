...
| 04 | `codex-obsidian` | 連接 Obsidian（資料夾授權 / MCPVault） |
| 05 | `codex-second-brain` | 第二大腦三層結構 |
| 06 | `codex-supabase` | 連接 Supabase |
| 07 | `codex-firebase` | 連接 Firebase |
| 08 | `codex-ollama` | 本地 AI Ollama |
| 09 | `codex-gemini` | Gemini 免費 API |
| 10 | `codex-workspace` | 專案初始化工作模式 |
| 11 | `codex-draw` | 生圖指引（內建 + API） |
| 12 | `codex-chezmoi` | 跨電腦同步 Codex 設定 |
| 13 | `codex-install-all` | 一次安裝全部 |
| 14 | `startup-sync` | 開工時唯讀載入專案規則、Obsidian 與 Git 狀態 |
| 15 | `shutdown-sync` | 收工時安全檢查、更新駕駛艙並自動提交推送至 `origin/master` |
| 16 | `project-init-sync` | 安全建立或補齊新專案工作區 |
...
```bash
npx skills add changyiwu/codex-lazy-packs --skill <完整 Skill 名稱> --full-depth -g -y
```
