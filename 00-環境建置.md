...
> 💡 **本懶人包只負責安裝基礎工具。** 本流程不檢查 GitHub 帳號、不執行 `gh auth status`，也不安裝 Git 或 GitHub CLI。需要連接 GitHub、把教材推上線時，再執行 **懶人包 #03：連接 GitHub**。
...
2. **網路連線**
3. **Node.js**（部分 MCP 工具需要）：`node --version`（必須使用仍受官方支援的 LTS，不要只用「版本大於 18」判斷）
...
### 步驟二：安裝 Node.js（如果未安裝、已停止支援或不是 LTS）

> 💡 不是 Codex Desktop app 本體需要的，但許多以 Node.js 執行的 MCP server 會用到。Gemini Notebook MCP（套件技術名稱仍為 `notebooklm-mcp-cli`）則是 Python 工具，主要依賴 uv。
...
uv 是 Python 套件管理工具，後續安裝 Gemini Notebook 等 MCP 工具會用到。
